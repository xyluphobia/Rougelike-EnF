...
public class RotatorPattern : MonoBehaviour
{
    ...
    public void BindHolder(string originalBindsPassthrough, string characterNamePassthrough)
    {
        if (originalBinds != null)
            return;

        originalBinds = originalBindsPassthrough;
        characterName = characterNamePassthrough;
    }
    ...
    private void OnDeath()
    {
        ...
        else if (characterName.Equals(GameAssets.i.MOBACharacter.name) || characterName.Equals(GameAssets.i.MOBACharacter.name + "(Clone)"))
        {
            ...
            playerInput.currentActionMap.FindAction("AbilityOne").ApplyBindingOverride($"<Keyboard>/{abilityOne}"); // wasdUp
            playerInput.currentActionMap.FindAction("AbilityTwo").ApplyBindingOverride($"<Keyboard>/{abilityTwo}"); // wasdDown
            playerInput.currentActionMap.FindAction("AbilityThree").ApplyBindingOverride($"<Keyboard>/{abilityThree}"); // wasdLeft
            playerInput.currentActionMap.FindAction("UltimateAbility").ApplyBindingOverride($"<Keyboard>/{ultimateAbility}"); // wasdRight
        }
    }
